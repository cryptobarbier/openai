...
class Sportify2(gym.Env):
    def __init__(self, xtrain,xtest,training,train_id,test_id,batch):
...
# Different actions Home(0) and bet size (20 40 60 80 100)
    ...
    def step(self, action):
            ...
            if acti>=0: # We buy
                if self.outcome==1:
                    return self._get_obs(), acti*(self.odds-1), finished, {}
                else:
                    return self._get_obs(), -acti, finished, {}
            else:

                if self.outcome==1:
                    return self._get_obs(),-acti, finished, {}
                else:
                    return self._get_obs(), acti*(1/(self.odds-1)), finished, {}          
